...
describe("buildResultViewModel", () => {
  it("formats user-budget amounts in USD while preserving model-native pricing", () => {
    ...
    expect(viewModel.costSection.sourceLabel).toBe("models.dev API")
    expect(viewModel.costSection.sourceNote).toContain("models.dev cost.input/output/cache_read")
    expect(viewModel.costSection.sourceSyncedAt).toBe("2026-03-05")
    ...
    expect(viewModel.dataDisclaimer.pricingSource).toContain("models.dev API")
    expect(viewModel.dataDisclaimer.pricingReferences.some((reference) => reference.sourceLabel === "models.dev API")).toBe(true)
    expect(viewModel.dataDisclaimer.pricingReferences.some((reference) => reference.sourceLabel === "Zhipu Open Platform Pricing")).toBe(true)
